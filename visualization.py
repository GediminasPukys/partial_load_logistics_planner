...
from utils import haversine_distance, format_time, format_duration, generate_color, truck_utilization, \
    calculate_empty_miles
...
def create_truck_tables(routes, selected_truck, warehouse_coords, orders_df=None):
    """
    ...
    Args:
        ...
        warehouse_coords: Coordinates of the warehouse
        orders_df: DataFrame with order information (optional)
    """
    ...
    # Prepare order details for lookup if provided
    order_details = {}
    ...
    for i, waypoint in enumerate(route['waypoints']):
        ...
        # Format location name and get time window
        if order_id == 'Warehouse':
            location = "Warehouse"
            time_window = "N/A"
        else:
            location = f"Order {order_id}"
            # Get time window if available
            if order_details and int(order_id) in order_details:
                order = order_details[int(order_id)]
                time_window = f"{order['Available_from'].strftime('%H:%M')}-{order['Available_to'].strftime('%H:%M')}"
            else:
                time_window = "Unknown"
